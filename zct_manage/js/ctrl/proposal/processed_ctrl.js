 ...
 */
define(["app", 'echarts', 'api/proposal_api','services/myalert/myalert','services/setting','services/common'], function (myapp, echarts) {
    myapp.controller('processed_ctrl', ['$scope','$rootScope','proposal_api','myalert','$state','setting','common',function (s,$rootScope, proposal_api,myalert,$state,setting,common) {
        $rootScope.istick = 'processed';
        //已审批
        s.getType = 2;
        //初始化搜索类型
        s.searchTypes = [
...
//分页函数
        ...
        var getProposals = function () {
            proposal_api.getProposals(s.send_obj).success(function (data) {
                console.log("000");
                console.log(data);
